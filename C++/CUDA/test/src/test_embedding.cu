...
int test_shuffled_joint_embedding() {
	bool match = true;
	...
	// Copy output vector from GPU buffer to host memory.
	float* host_state_space_Z = new float[6 * 3];

	cudaStatus = cudaMemcpy(host_state_space_Z, dev_state_space_Z, 18 * sizeof(float), cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
		match = false;
		goto Error;
	...
	for (int i = 0; i < 6 * 3; i++) {
		if (!floats_equal(expected_state_space[i], host_state_space_Z[i])) {
			match = false;
			break;
		}
	}
...
int main() {
	...
	cout << "Test shuffled joint embedding failed: " << test_shuffled_joint_embedding() << endl;
